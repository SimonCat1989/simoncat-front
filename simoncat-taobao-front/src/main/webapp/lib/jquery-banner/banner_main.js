  ...
  function slide(slideMode) {// 轮播方法
    if (isMoving == false) {
      ...
      if (index_2 == len)
        index_2 = 0;
      if (index_2 < 0)
        index_2 = len + index_2; // 限制index_2的范围
      $(".btn_list span").removeClass('curr').eq(index_2).addClass('curr');// 给序列号按钮添加、移除样式
    }
  }// slide()

  /**
   * Only when the visible banner need to be initialized, otherwise, let's stop
   * them to improve performance.
   */
  for ( var $banner in $(".main_banner")) {
    if ((jQuery(window).scrollTop() > ($banner.offset().top + $banner.outerHeight()))
        || ((jQuery(window).scrollTop() + jQuery(window).height()) < $banner.offset().top)) {
  ...
  if (len > 3) {
    ...
    $(".btn_list span").click(
        function(event) {
          ...
          if (oIndex == index_2)
            return;// 点击按钮的序列号与当前图片的序列号一致，return
          ...
          if (Math.abs(index_2 - oIndex) > 1 && Math.abs(len - Math.abs(index_2 - oIndex)) != 1) {
            ...
            if ((index_2 > oIndex && len - (index_2 - oIndex) != 1)
                || (index_2 < oIndex && len + (index_2 - oIndex) == 1)) {
              flag = true; // 执行上一张
            }
          }
          slide(flag);
