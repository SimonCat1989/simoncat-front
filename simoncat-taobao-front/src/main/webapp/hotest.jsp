    ...
    <div class="inner">
      ...
      <div class="pagination">
        <c:if test="${essays.getCurrentPage() > 1}">
          <a href="home/hotest.do?page=${essays.getCurrentPage() - 1}" class="button previous">上一页</a>
        </c:if>
        <div class="pages">
          <c:forEach var="pageCount" begin="1" end="${essays.getTotalPage()}" step="1">
            <c:choose>
              <c:when test="${pageCount == essays.getCurrentPage()}">
                <a href="home/hotest.do?page=${pageCount}" class="active">${pageCount}</a>
              </c:when>
              <c:otherwise>
                <a href="home/hotest.do?page=${pageCount}">${pageCount}</a>
              </c:otherwise>
            </c:choose>
          </c:forEach>
        </div>
        <c:if test="${essays.getCurrentPage() < essays.getTotalPage()}">
          <a href="home/hotest.do?page=${essays.getCurrentPage() + 1}" class="button next">下一页</a>
        </c:if>
      </div>
    </div>
  </div>
